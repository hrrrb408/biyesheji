...
本目录包含用于放射组学二分类的 Logistic Regression (LR) 建模流水线，覆盖 **CT**、**Dose** 以及 **多模态早期融合（Early Fusion，特征拼接）**。
...
## 3. 输出产物
- 单次三划分：`LR/omics/<task>/metric_auc/` 下生成 `test_predictions.csv`、`train_oof_predictions.csv`、`test_metrics.json`、`best_params.json` 等

## 4. 目录结构
- `lr_train.py`：训练与评估主脚本（单次三划分）。
- `omics/`：各任务输出目录与汇总 CSV。
